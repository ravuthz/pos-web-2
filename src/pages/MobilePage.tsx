...
export function MobilePage() {
  ...
  const categoriesQuery = useQuery({
    ...
    queryFn: async () => {
      const response = await categoryService.getAll({ per_page: 200, sort_by: 'name', sort_order: 'asc' });
  ...
  const productsQuery = useQuery({
    queryKey: ['mobile-products', selectedBranchId, activeCategoryId],
    queryFn: async () => {
      if (!selectedBranchId || !activeCategoryId) {
        return [];
      }
    ...
    enabled: Boolean(selectedBranchId && activeCategoryId)
  });
  ...
  if (currentShiftQuery.isLoading || categoriesQuery.isLoading) {
  ...
  return (
    <>
      <div className="space-y-4 pb-28">
        ...
        {!currentShiftQuery.data ? (
          ...
          <EmptyState
            ...
            message="Create categories first so products can be grouped in the mobile order screen."
          />
        ) : (
          <>
            <section className="sticky top-0 z-20 -mx-4 border-y border-base-300 bg-base-100/95 px-4 py-3 backdrop-blur lg:-mx-6 lg:px-6">
              <div role="tablist" className="tabs flex-nowrap gap-2 overflow-x-auto p-0 scrollbar-hide">
                {categories.map((category) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      role="tab"
                      className={`tab h-11 shrink-0 rounded-[calc(var(--radius-box)*1.2)] border px-4 ${isActive
            ...
            <section className="space-y-4">
              {productsQuery.isLoading && !productsQuery.data ? (
                ...
                <EmptyState
                  ...
                  message="Try another category or add products to this one first."
                />
              ) : (
                <div className="grid grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
                  {products.map((product) => (
                    <button
                      key={product.id}
                      type="button"
                      className="card overflow-hidden border border-base-300 bg-base-100 text-left shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-primary hover:shadow-md"
                    ...
                    >
                      <figure className="aspect-square bg-base-200">
                        {product.image_url ? (
                          ...
                          <div className="flex h-full w-full items-center justify-center bg-primary/8 text-2xl font-semibold text-primary/70">
                      ...
                      <div className="card-body gap-2 p-4">
                        <div className="space-y-1">
                          <h3 className="line-clamp-2 font-medium text-surface-900">{product.name}</h3>
                          <p className="text-xs text-surface-500">{product.code ?? product.barcode ?? 'No code'}</p>
                        </div>
            ...
            <div className="sticky bottom-0 z-20 pt-4">
              <div className="dock dock-md !static overflow-hidden rounded-[calc(var(--radius-box)*1.4)] border border-base-300 bg-base-100 shadow-lg">
                <button
                  type="button"
                  className={isSearchModalOpen ? 'dock-active text-primary' : ''}
                ...
                <button
                  type="button"
                  className={`relative ${isCartModalOpen ? 'dock-active text-primary' : ''}`}
                  onClick={() => setIsCartModalOpen(true)}
                >
                  {uniqueProductCount > 0 ? (
                    <span className="badge badge-primary badge-sm absolute right-3 top-1.5">
                      {uniqueProductCount}
                    </span>
                  ) : null}
      ...
      {isSearchModalOpen ? (
        <div className="fixed inset-0 z-40 flex items-end justify-center bg-neutral/45 p-4 sm:items-center">
      ...
      {isCartModalOpen ? (
        <div className="safe-top safe-bottom fixed inset-0 z-40 flex items-end justify-center bg-neutral/45 px-4 pb-4 pt-20 sm:items-center sm:p-5">
          <div className="card flex max-h-full min-h-0 w-full max-w-3xl self-end flex-col overflow-hidden border border-base-300 bg-base-100 shadow-2xl sm:max-h-[88vh] sm:self-auto mt-10">
            <div className="sticky top-0 z-10 flex shrink-0 items-center justify-between border-b border-base-300 bg-base-100 px-5 py-4">
              <div>
                ...
                <p className="text-sm text-surface-500">Review quantities and complete the order.</p>
              </div>
              <button
                type="button"
                className="btn btn-ghost btn-sm btn-square"
            ...
            <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain">
              <div className="space-y-4 p-5">
                {cart.length === 0 ? (
                  <EmptyState
                    title="Cart is empty"
                    message="Add products from the category grid or search popup to build the order."
                  />
                ) : (
                  <>
